...
torch.cuda.set_per_process_memory_fraction(0.9)  # Allocate 90% of the GPU memory
...
class IPHI(nn.Module):
    def __init__(self, width=32):
        ...
        self.fc_code = nn.Linear(4, self.width)
...
################################################################
PATH = '/home/hcheng/GrainLearning/grainLearning/tutorials/physics_based/2D_dike_stability/data_gen'
PATH_Sigma = PATH+'/output_features.npy'
PATH_XY = PATH+'/input_coords.npy'
PATH_rr = PATH+'/input_features.npy'

input_features = np.load(PATH_rr) # phi, psi, coh, ks
input_rr = torch.tensor(input_features, dtype=torch.float)
...
################################################################
model = FNO2d(modes, modes, width, in_channels=2, out_channels=2).cuda()
...
for ep in range(epochs):
    ...
    #     truth = sigma[-1].squeeze().detach().cpu().numpy()[:,1]
    #     pred = out[-1].squeeze().detach().cpu().numpy()[:,1]
    #
    #     lims = dict(cmap='RdBu_r', vmin=truth.min(), vmax=truth.max())
    #     fig, ax = plt.subplots(nrows=1, ncols=3, figsize=(12, 4))
    #     im0 = ax[0].scatter(XY[:, 0], XY[:, 1], 100, truth, edgecolor='w', lw=0.1, **lims)
    #     im1 = ax[1].scatter(XY[:, 0], XY[:, 1], 100, pred, edgecolor='w', lw=0.1, **lims)
    #     im2 = ax[2].scatter(XY[:, 0], XY[:, 1], 100, truth - pred, edgecolor='w', lw=0.1, **lims)
